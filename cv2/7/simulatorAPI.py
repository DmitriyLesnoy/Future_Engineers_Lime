...
class ConnectorDirect():
    ...
    def __init__(self, back_plane_ip_address=IP):
        ...
        self.min_pause_betwen_packs = 0.02
                ...
                if byte_frame!=b'':
                    for b in byte_frame:
                        if b==126:
                            # print(pack)
                            pack = eval(pack)
                            self.incoming_message_processing("", pack)
                            pack=bytearray()
                        else:
                            pack.append(b)
        ...
        while True:
            if len(self.list_messages)>0:
                ...
                if flag:
                    ...
                    self.timer_last_send=time.time()+self.min_pause_betwen_packs
                else:
...
class Camera():
    ...
    def frame(self, wait_new_frame=True):
        ...
        if self.block.update_data is not None:# and self.block.update_flag==False:
...
class Gyro():
    ...
    def add_torque_y(self, y, t=100):
        self.block.send_command(f"'c':'y', 'v':{y}, 'w':{t}", flag_garanted_send=True)
...
class Sensor():
    def __init__(self, block):
        self.block = block
        self._speed = 0
        self._position = {'x': None, 'y': None, 'z': None}
        self._rotation = {'x': None, 'y': None, 'z': None, 'w': None}
        self._velocity = {'x': None, 'y': None, 'z': None}
        self.started=False
    ...
    def ser(self):
        ...
        if self.block.update_data is not None:
            ...
            self._speed = list_f[0]
            self._position = {'x': list_f[1], 'y': list_f[2], 'z': list_f[3]}
            self._rotation = {'x': list_f[4], 'y': list_f[5], 'z': list_f[6], 'w': list_f[7]}
            self._velocity = {'x': list_f[8], 'y': list_f[9], 'z': list_f[10]}
            # return
            self.block.update_data = None
...
class Painter():
    def __init__(self, block):
        self.block = block
        self.colors=['clear', "red", "blue", "green", "white"]
    ...
    def color(self, color_name):
        # берем индекс класса по названию
        class_index=1
        try:
            class_index = self.colors.index(color_name)
        except:
            print("No color", color_name, "in painter!. Set red")
        self.block.send_command(f"'c':'a', 'v':'{class_index}'", flag_garanted_send=True)
...
class ConnectorBlock():
    ...
    def drop(self, id):
        self.block.send_command(f"'c':'d', 'v':{id}", flag_garanted_send=True)

class DroneControllerBlock():
    def __init__(self, block):
        self.block = block
    ...
    def up(self, value, timer=100):
        self.block.send_command(f"'c':'u', 'v':{value}, 'w':{timer}", flag_garanted_send=True)
        time.sleep(0.01)

    def down(self, value, timer=100):
        self.block.send_command(f"'c':'d', 'v':{value}, 'w':{timer}", flag_garanted_send=True)
        time.sleep(0.01)

    def forward(self, value, timer=100):
        self.block.send_command(f"'c':'f', 'v':{value}, 'w':{timer}", flag_garanted_send=True)
        time.sleep(0.005)

    def back(self, value, timer=100):
        self.block.send_command(f"'c':'b', 'v':{value}, 'w':{timer}", flag_garanted_send=True)
        time.sleep(0.005)

    def left(self, value, timer=100):
        self.block.send_command(f"'c':'l', 'v':{value}, 'w':{timer}", flag_garanted_send=True)
        time.sleep(0.005)

    def right(self, value, timer=100):
        self.block.send_command(f"'c':'r', 'v':{value}, 'w':{timer}", flag_garanted_send=True)
        time.sleep(0.005)
...
class base_robot():
    ...
    def angle_to_point(self, position, target):
        x_point, y_point = target
        rotation = self.sensor.get_rotation()["y"]
        x, y = position
        ...
        a2 = self.wrap_angle_360(rotation)

        a3 = self.LineAngle(x_point, y_point, x, y)

        return self.wrap_angle_360(180 + a3 - a2 + 90) - 180
...
class UnityAPI():
    def __init__(self, back_plane_ip_address=None):
        ...
        # break
    def connect_direct_video_socket(self):
        try:
    ...
    def direct_video_work(self):
        self.connect_direct_video_socket()
        # self.mySocket_video = socket.socket()
        # self.mySocket_video.connect((IP, 9092))
        while 1:
            try:
    ...
    def get_robot(self, name, id_grid=None):
        for grid in self.grids:
            if grid.name == name:
                if id_grid is None:
                    return grid
                else:
                    if int(id_grid)==grid.id:
                        return grid
        return None

    def make_cv_robot(self, name="Robot", id_grid=None, camera=1):
        data = self.get_robot(name, id_grid=id_grid)
        robot = RobotCV(data, name=name, camera=camera)
        robot.sleep(100)
        return robot
    ...
    def make_car_robot(self, name="Robot"):
        ...
        robot.sleep(100)
        return robot
    def make_drone_robot(self, name="Drone"):
        data = self.get_robot(name)
        robot = DroneCV(data, name=name)
        robot.sleep(100)
        return robot
...
class RobotCV(base_robot):
    def __init__(self, car, name, camera=1):
        self.name = name
        ...
        self.painter = None
        try:
            self.painter = Painter(car.get_block_by_name("Painter"))
            # self.container.content()
        except:
            pass

        self.on_all(camera=camera)

    def on_all(self, camera):
...
class DroneCV(base_robot):
    def __init__(self, drone, name, camera=1):
        self.name = name
        self.DroneController = DroneControllerBlock(drone.get_block_by_name("DroneController"))
        self.engine1 = Engine(drone.get_block_by_name("Engine1"))
        self.engine2 = Engine(drone.get_block_by_name("Engine2"))
        self.engine3 = Engine(drone.get_block_by_name("Engine3"))
        self.engine4 = Engine(drone.get_block_by_name("Engine4"))
        self.engine5 = Engine(drone.get_block_by_name("Engine5"))
        self.engine6 = Engine(drone.get_block_by_name("Engine6"))
        self.engine7 = Engine(drone.get_block_by_name("Engine7"))
        self.engine8 = Engine(drone.get_block_by_name("Engine8"))
        self.engine9 = Engine(drone.get_block_by_name("Engine9"))
        self.engine10 = Engine(drone.get_block_by_name("Engine10"))

        self.camera = Camera(drone.get_block_by_name("CameraBlock"))
        self.sensor = Sensor(drone.get_block_by_name("SensorBlock"))
        # self.lamp = Lamp(car.get_block_by_name("Lamp"))
        # self.rotor0 = Rotor(car.get_block_by_name("Rotor"))
        ...
        self.container = None
        try:
            self.container = Container(drone.get_block_by_name("Container"))
            # self.container.content()
        except:
            pass
        self.connector1 = None
        try:
            self.connector1 = ConnectorBlock(drone.get_block_by_name("Connector"))
            self.connector1.power("on")
            # self.container.content()
        except:
            pass
        self.gyro = None
        try:
            self.gyro = Gyro(drone.get_block_by_name("Gyro"))
            self.gyro.power("on")
            # self.container.content()
        except:
            pass
        ...
        try:
            self.laser_sensor = LaserSensor(drone.get_block_by_name("LaserSensor"))
            self.laser_sensor.power("on")
