...
low = np.array([0, 0, 0])
up = np.array([256, 256, 70])

low_g = np.array([40, 150, 80])
up_g = np.array([80, 256, 256])
timer_g = 0
count_g = 0
flag_g = False
while True:
    ...
    if faza == 1:
        ...
        if flag_move:
            frameG = frame[Y1g:Y2g, X1g:X2g].copy()
            ...
            for contur in contours:
                ...
                if cv2.contourArea(contur) > 500:
                    flag_g = True
                    break
            if flag_g and time.time() > timer_g:
                timer_g = time.time() + 2
                count_g += 1
                robot.beep()
            ...
            if count_g >= 3:
                flag_move = False
                robot.serv(0)
                robot.move(100, 0, 300)
                robot.move(-100, 0, 300)
                robot.sound1()
                # robot.move(-255, 0, 300)
        #     print(flag_g)
        ...
        if min_x_r != 100000:
            ...
            reg_move.set(0.2, 0.02, 0.01)  # (0.2, 0.01, 0.01), (0.2, 0.02, 0.001)б (0.2, 0.01, 0.015)
        ...
        if flag_show:
            ...
            cv2.rectangle(frame_show, (X1g, Y1g), (X2g, Y2g), c, 2)
